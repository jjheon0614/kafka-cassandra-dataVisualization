...
curl -s \
  ...
  "config": {
    ...
    "contactPoints": "cassandradb",
    "loadBalancing.localDc": "datacenter1",
    "topic.faker.kafkapipeline.fakerdata.mapping": "name=value.name, address=value.address, year=value.year, phone_number=value.phone_number, job=value.job, email=value.email, company=value.company, city=value.city, state=value.state, country=value.country",
    "topic.faker.kafkapipeline.fakerdata.consistencyLevel": "LOCAL_QUORUM"
  }
}'
echo "Starting Crypto API"
curl -s \
     -X POST http://localhost:8083/connectors \
     -H "Content-Type: application/json" \
     -d '{
  ...
  "config": {
    ...
    "contactPoints": "cassandradb",
    "loadBalancing.localDc": "datacenter1",
    "topic.crypto.kafkapipeline.cryptodata.mapping": "date=value.time, btc=value.btc, eth=value.eth, doge=value.doge",
    "topic.crypto.kafkapipeline.cryptodata.consistencyLevel": "LOCAL_QUORUM"
  }
}'
echo "Done."
