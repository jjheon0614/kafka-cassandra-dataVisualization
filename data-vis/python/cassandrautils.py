...
cryptotable = os.getenv("cryptotable.table", "cryptodata")
...
CRYPTO_TABLE = os.environ.get("CRYPTO_TABLE") if os.environ.get("CRYPTO_TABLE") else 'crypto'
...
def saveFakerDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + fakertable + " (name, address, year, phone_number, job, email, company, city, state, country) \
                   VALUES (?, ?, ?)"
    batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
    insert = session.prepare(cqlsentence)
    batches = []
...
def saveCryptoDf(dfrecords):
    ...
    cqlsentence = "INSERT INTO " + cryptotable + " (btc, date, doge, eth) \
                   VALUES (?, ?, ?, ?)"
    batch = BatchStatement(consistency_level=ConsistencyLevel.QUORUM)
    insert = session.prepare(cqlsentence)
    batches = []
    for idx, val in dfrecords.iterrows():
        batch.add(insert, (val['btc'], val['date'],
                           val['doge'], val['eth']))
        counter += 1
        if counter >= 100:
            print('inserting ' + str(counter) + ' records')
...
def loadDF(targetfile, target):
    if target == 'weather':
        ...
        colsnames = ['name', 'address', 'year', 'phone_number', 'job', 'email', 'company', 'city', 'state', 'country']
        dfData = pd.read_csv(targetfile, header=None,
                             parse_dates=True, names=colsnames)
        saveFakerDf(dfData)
    elif target == 'crypto':
        colsnames = ['btc', 'date', 'doge', 'eth']
        dfData = pd.read_csv(targetfile, header=None,
                             parse_dates=True, names=colsnames)
        saveCryptoDf(dfData)
...
def getDF(source_table):
    ...
    if source_table not in (WEATHER_TABLE, TWITTER_TABLE, FAKER_TABLE, CRYPTO_TABLE):
        return None
